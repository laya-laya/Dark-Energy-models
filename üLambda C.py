...
def integrand1(z,omgM,z_star):
    return np.sqrt(1-((0.75*omgM)/(1+z_star))+((0.75*omgM)/(1+z_star))*((1+z)**4))

def integrand2(z,omgM,z_star):
    return np.sqrt((1-omgM)+ (omgM * np.power(1 + z,3)))


def luminosity_distance1(z,omgM,h,z_star):
    return (((quad(integrand1,0, z_star,args=(omgM,z_star))[0])/(1+z)))*4.3*(10**3)

def luminosity_distance2(z,omgM,h,z_star):
    return (((quad(integrand2,z_star, z,args=(omgM,z_star))[0])/(1+z)))*4.3*(10**3)

def luminosity_distance(z,omgM,h,z_star):
    
    return luminosity_distance1(z,omgM,h,z_star)+luminosity_distance2(z,omgM,h,z_star)
    
"""
def luminosity_distance(z,omgM,h,z_star):
    #h = h_0(omgM)
    integral, _ = quad(E, 0, z, epsrel=1e-8, args=(omgM,z_star))
    return (cte.c /10**5) /h*(1+z) *integral
"""
def distance_modulus(z,omgM,h,z_star):
    #h = h_0(omgM)
    return 5. * np.log10(luminosity_distance(z,omgM,h,z_star)) +25
...
def chisq_sn(omgM,h,z_star):
    
    m_model = np.array([distance_modulus(z,omgM,h,z_star) for z in z_sn])
...
def chisq_sn_cmb(pars):
    ...
    omgM,h,gamma0,gamma1,sigma8,z_star = pars
    
    return chisq_sn(omgM,h,z_star)+ chisq_fsigma8(sigma8,omgM,gamma0,gamma1)+chisq_cmb2(omgM,h)
           
result_sn_cmb = minimize(chisq_sn_cmb, [omgM_fid,h_fid,gamma0_fid,gamma1_fid,sigma8_fid,z_star_fid], bounds=((0.01, 1.), (0.01, 1.),(0.1,1),(-1,1),(0.25,1.25),(0.1, 1.)))
omgM_sn_cmb,h_sn_cmb,gamma0_sn_cmb ,gamma1_sn_cmb,sigma8_sn_cmb,z_star_sn_cmb = result_sn_cmb.x
...
print("z_star = ", z_star_sn_cmb)

def lnprior(pars):
    h, omgM,gamma0,gamma1,sigma8,z_star = pars
    if  0.0 < omgM < 1.0 and  0.0 < h and 0.0 < gamma0 < 1 and 0 < sigma8 < 2 and -1 < gamma1 < 1 and 0.1 < z_star < 1:
        return 0.0
    return -np.inf

def lnlike_sn_bao(pars):
    omgM,h,gamma0,gamma1,sigma8,z_star  = pars
    return -0.5 * chisq_sn_cmb([omgM,h, gamma0,gamma1 , sigma8,z_star ])
...
ndim, nwalkers, nsteps = 6, 50, 1000
pos = [[omgM_sn_cmb,h_sn_cmb,gamma0_sn_cmb,gamma1_sn_cmb,sigma8_sn_cmb,z_star_sn_cmb] + 1e-4*np.random.randn(ndim) for i in range(nwalkers)]
...
z_star_sn_cmb_chain = sampler.chain[:,:,5]
...
z_star_sn_cmb_chain_flat = np.reshape(z_star_sn_cmb_chain, (nwalkers*nsteps,))

labels = [r"$\Omega_{m0}$", r"$h$",r"$gamma_0$",r"$gamma_1$",r"$sigma8$",r"$z_star$"]
samples = np.c_[omgM_sn_cmb_chain_flat,h_sn_cmb_chain_flat,gamma0_sn_cmb_chain_flat,gamma1_sn_cmb_chain_flat,sigma8_sn_cmb_chain_flat,z_star_sn_cmb_chain_flat].T
...
z_star_sn_cmb_chain_mean = np.mean(z_star_sn_cmb_chain, axis=0)
z_star_sn_cmb_chain_err = np.std(z_star_sn_cmb_chain, axis=0) / np.sqrt(nwalkers)
